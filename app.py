...
cred_path = (
    ...
    else 'firebase_credentials.json'
)
...
def disparar_email_boas_vindas(email, nome, senha):
  ...
  return enviar_email_html(
      email, 'Bem-vindo(a) ao FACSS CRM - Confirmação de Acesso', corpo
  )
...
def disparar_email_recuperacao(email, nome, senha):
  corpo = f"""
    ...
    """
  return enviar_email_html(
      email, 'Recuperação de Senha - FACSS CRM', corpo
  )
...
def clientes():
  ...
  for c in raw_clientes:
    if busca:
      if not (
          busca in str(c.get('nome_empresa', '')).lower()
          or busca in str(c.get('cnpj', '')).lower()
          or busca in str(c.get('responsavel', '')).lower()
      ):
        continue
    lista.append((
        c['_id'],
  ...
  return render_template(
      'clientes.html', clientes=lista, active_tab='clientes', busca=busca
  )


@app.route('/novo_cliente')
def novo_cliente():
  return render_template('cadastro_cliente.html', active_tab='clientes')
...
def editar_cliente(id):
  ...
  return render_template(
      'editar_cliente.html', cliente=cliente, active_tab='clientes'
  )
  ...
  if db:
    try:
      ...
      db.collection('tb_clientes').document(str(id_cliente)).update({
          'nome_empresa': (
              request.form.get('nome') or request.form.get('nome_empresa')
          ),
          'cnpj': request.form.get('cnpj'),
          'contato_nome': (
              request.form.get('contato') or request.form.get('contato_nome')
          ),
          'telefone': request.form.get('telefone'),
          'email': request.form.get('email'),
          ...
          'responsavel': request.form.get('responsavel'),
          'health_score': (
              request.form.get('health') or request.form.get('health_score')
          ),
          'faturamento': request.form.get('faturamento'),
          'nivel_acesso': request.form.get('nivel_acesso'),
      })
      ...
      print(f'[ERRO ATUALIZAR CLIENTE] {e}')
  return redirect('/clientes')
...
def ficha_cliente(id):
  ...
  interacoes = [
      ...
      for i in get_collection('tb_interacoes')
      if str(i.get('id_cliente')) == str(id)
  ]
  modulos = [
      (
          m['_id'],
          ...
          m.get('marca'),
          m.get('modelo'),
          m.get('id_modulo_equip'),
          m.get('chip'),
          m.get('num_chip'),
          m.get('data_instalacao'),
          m.get('data_desativacao'),
          m.get('situacao'),
      )
      for m in get_collection('tb_modulos')
      if str(m.get('id_cliente')) == str(id)
  ]
  return render_template(
      'ficha_cliente.html',
      cliente=cliente,
...
def modulos():
  ...
  clientes_dict = {
      c['_id']: c.get('nome_empresa') for c in get_collection('tb_clientes')
  }
  modulos_tratados = []
  for m in raw_modulos:
    ...
    modulos_tratados.append({
        'id': m['_id'],
        'placa_raw': placa_raw,
        'placa_limpa': placa_raw,
        'padrao_identificador': placa_raw,
        'marca_modelo': (
            f"{m.get('marca', '')} {m.get('modelo', '')}".strip()
        ),
        'id_equip': str(m.get('id_modulo_equip', '')),
        'cliente': clientes_dict.get(
            str(m.get('id_cliente')), 'Estoque / Sem vínculo'
        ),
        'situacao': str(m.get('situacao', 'ATIVO')).upper(),
    })
  return render_template(
      'modulos.html', modulos=modulos_tratados, active_tab='modulos'
  )
...
def novo_modulo():
  ...
  return render_template(
      'cadastro_modulo.html', clientes=clientes, active_tab='modulos'
  )
  ...
  if db:
    try:
      ...
      db.collection('tb_modulos').document(str(id_modulo)).update({
          ...
          'num_chip': request.form.get('num_chip', ''),
          'data_instalacao': request.form.get('data_instalacao', ''),
          'data_desativacao': request.form.get('data_desativacao', ''),
          'situacao': request.form.get('situacao', 'ATIVO'),
      })
    except Exception as e:
      print(f'[ERRO ATUALIZAR MODULO] {e}')
  return redirect('/modulos')
...
def pipeline():
  ...
  for lead in lista_leads:
    ...
    item = (
        ...
        lead.get('segmento'),
        lead.get('decisor'),
        lead.get('canal_aquisicao'),
        lead.get('estagio'),
        lead.get('proxima_acao'),
    )
    if 'contato' in estagio:
      funil['Contato Feito'].append(item)
    elif 'diagn' in estagio or 'reuni' in estagio:
      funil['Diagnóstico / Reunião'].append(item)
    elif 'proposta' in estagio:
      funil['Proposta Enviada'].append(item)
    elif 'negoc' in estagio or 'fechado' in estagio:
      funil['Negociação'].append(item)
    else:
      funil['Leads Iniciais'].append(item)
  return render_template(
      'pipeline_kanban.html', funil=funil, active_tab='kanban'
  )


@app.route('/novo_lead')
def novo_lead():
  return render_template('cadastro_lead.html', active_tab='kanban')
  ...
  if db:
    try:
      ...
      doc_ref.set({
          ...
          'canal_aquisicao': request.form.get('canal', ''),
          'estagio': request.form.get('estagio', ''),
          'proxima_acao': request.form.get('proxima', ''),
      })
    except Exception as e:
      print(f'[ERRO SALVAR LEAD] {e}')
  ...
  if db:
    try:
      ...
      if doc.exists:
        ...
        if direcao == 'frente' and idx < len(ordem) - 1:
          novo = ordem[idx + 1]
        elif direcao == 'tras' and idx > 0:
          novo = ordem[idx - 1]
        else:
          novo = estagio_atual
        doc_ref.update({'estagio': novo})
    except Exception as e:
      print(f'[ERRO MOVER LEAD] {e}')
  return redirect('/pipeline')
...
def ordens_servico():
  ...
  lista = []
  for o in raw_os:
    c_nome = clientes_dict.get(
        str(o.get('id_cliente')), 'Cliente Indefinido'
    )
    lista.append((
        o['_id'],
        o.get('numero_os'),
...
def nova_os():
  ...
  analistas = [
      ...
      for u in get_collection('tb_usuarios')
...
def salvar_os():
  ...
  if db and id_cliente:
    try:
      ...
      print(f'[ERRO BUSCAR CLIENTE OS] {e}')

  usuarios = get_collection('tb_usuarios')
  a_email = next(
      (u.get('email') for u in usuarios if u.get('nome') == analista_nome), None
  )

  destinatarios = [e for e in [c_email, a_email, EMAIL_EMPRESA] if e]
  if destinatarios:
    corpo_os = f"""
        <html>
            <body style="font-family: Arial, sans-serif; background-color: #f8fafc; padding: 20px;">
    ...
    enviar_email_html(
        destinatarios,
        f'Nova Ordem de Serviço: OS #{numero_os} - FACSS',
        corpo_os,
    )
...
def imprimir_os(id):
  ...
  if db and o.get('id_cliente'):
    try:
      ...
      print(f'[ERRO IMPRIMIR OS CLIENTE] {e}')
